...
$Nome = "Marco";	# Solo per le prove
$Cognome = "Cavalieri";	# Solo per le prove
	...
	open(BODY, "<$PathMod\\bodymsg.html") or die "++ File body inesistente in $PathMod , $!";
	while(<BODY>){
   		$body = $body . $_;
	}
	$body =~ s/---Nome---/$Nome/;
	...
	if  (defined $Email and $opts{email}) {
		...
		$sender->OpenMultipart( {	
			to      => "maurizio.cavalieri\@gmail.com",	# Solo per le prove
			#to      => "$Email",
		...
		encoding 	=> 'Base64',
		file 		=> "T:/Ricevute/Pagamenti/RicPP_2017_11.pdf",
		#file 		=> "$PathTipo/RicPP_2017_11.pdf",
